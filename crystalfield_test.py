...
class LS():
    ...
    def Sym(self,L,S):
        elements = LS(L,S)
        O20 = elements.Olm(L,S,2,0)
        O21m = elements.Olm(L,S,2,-1)
        O21p = elements.Olm(L,S,2,1)
        O22m = elements.Olm(L,S,2,-2)
        O22p = elements.Olm(L,S,2,2)
        O40 = elements.Olm(L,S,4,0)
        O41m = elements.Olm(L,S,4,-1)
        O41p = elements.Olm(L,S,4,1)
        O42m = elements.Olm(L,S,4,-2)
        O42p = elements.Olm(L,S,4,2)
        O43m = elements.Olm(L,S,4,-3)
        O43p = elements.Olm(L,S,4,3)
        O44m = elements.Olm(L,S,4,-4)
        O44p = elements.Olm(L,S,4,4)
        
        O60 = elements.Olm(L,S,6,0)
        O61m = elements.Olm(L,S,6,-1)
        O61p = elements.Olm(L,S,6,1)
        O62m = elements.Olm(L,S,6,-2)
        O62p = elements.Olm(L,S,6,2)
        O63m = elements.Olm(L,S,6,-3)
        O63p = elements.Olm(L,S,6,3)
        O64m = elements.Olm(L,S,6,-4)
        O64p = elements.Olm(L,S,6,4)
        O65m = elements.Olm(L,S,6,-5)
        O65p = elements.Olm(L,S,6,5)
        O66m = elements.Olm(L,S,6,-6)
        O66p = elements.Olm(L,S,6,6)
        
        #if [sym] == ['Ci'] or ['C1']: ## All of the parameters
            
            
        #elif [sym] == ['C2'] or ['Cs'] or ['C2h']:
            
        #elif [sym] == ['C2v'] or ['D2'] or ['D2h']:
            
        #elif [sym] == ['C4'] or ['S4'] or ['C4h']:
            
        #elif [sym] == ['D4'] or ['C4v'] or ['D2d'] or ['D4h']:
        
        #elif [sym] == ['C3'] or ['S6']:
        
        #elif [sym] == ['D3'] or ['C3v'] or ['D3d']:
        
        #elif [sym] == ['C6'] or ['C3h'] or ['C6h']:
        
        #elif [sym] == ['D6'] or ['C6v'] or ['D3h'] or ['D6h']:
        
        #elif [sym] == ['T'] or ['Th']:
        
        #elif [sym] == ['Td'] or ['O'] or ['Oh']:
    ...
    def PC(self,ion,L,S,d,Z):
        ...
        r=[]
        for k in range(len(d)):
            r = np.append(r,np.sqrt(x[k]**2 + y[k]**2 + z[k]**2))
        ...
        Zs22 = (-Z/5)*(1/4)*np.sqrt(15/np.pi)*(2*x*y/r**5)
        Zs21 = (-Z/5)*(1/2)*np.sqrt(15/np.pi)*(y*z/r**5)
        Z20 = -(Z/5)*(1/4)*np.sqrt(5/np.pi)*(3*z**2 - r**2)/r**5
        Zc21 = (-Z/5)*(1/2)*np.sqrt(15/np.pi)*(x*z/r**5)
        Zc22 = (-Z/5)*(1/4)*np.sqrt(15/np.pi)*((x**2 - y**2)/r**5)

        Zs44 = (-Z/9)*(3/16)*np.sqrt(35/np.pi)*(4*(x**3*y - x*y**3)/r**9)
        Zs43 = (-Z/9)*(3/8)*np.sqrt(70/np.pi)*((3*x**2*y - y**3)*z/r**9)
        Zs42 = (-Z/9)*(3/8)*np.sqrt(5/np.pi)*(2*x*y*(7*z**2 - r**2)/r**9)
        Zs41 = (-Z/9)*(3/4)*np.sqrt(5/(2*np.pi))*(y*z*(7*z**2 - 3*r**2)/r**9)
        Z40 =  (-Z/9)*(3/16)*np.sqrt(1/np.pi)*((35*z**4 - 30*z**2*r**2 + 3*r**4)/r**9)
        Zc41 = (-Z/9)*(3/4)*np.sqrt(5/(2*np.pi))*(x*z*(7*z**2 - 3*r**2)/r**9)
        Zc42 = (-Z/9)*(3/8)*np.sqrt(5/np.pi)*((x**2 - y**2)*(7*z**2 - r**2)/r**9)
        Zc43 = (-Z/9)*(3/8)*np.sqrt(70/np.pi)*((x**3 - 3*x*y**2)*z/r**9)
        Zc44 = (-Z/9)*(3/16)*np.sqrt(35/np.pi)*((x**4 - 6*x**2*y**2 + y**4)/r**9)

        """
        Zs66= (-Z/13)*(231/64)*np.sqrt(26/(231*np.pi))*((6*x**5*y - 20*x**3*y**3 + 6*x*y**5)/r**13)
        Zs65 = (-Z/13)*np.sqrt(9009/(512*np.pi))*((5*x**4*y - 10*x**2*y**3 + y**5)*z/r**13)
        Zs64 = (-Z/13)*(21/32)*np.sqrt(13/(7*np.pi))*(4*(x**3*y - x*y**3)*(11*z**2 - r**2)/r**13)
        Zs63 = (-Z/13)*(1/32)*np.sqrt(2730/np.pi)*((3*x**2*y - y**3)*(11*z**3 - 3*x*r**2)/r**13)
        Zs62 = (-Z/13)*(1/64)*np.sqrt(2730/np.pi)*(2*x*y*(33*z**4 - 18*z**2*r**2 + r**4)/r**13)
        Zs61 = (-Z/13)*(1/8)*np.sqrt(273/(4*np.pi))*(y*z*(33*z**4 - 30*z**2*r**2 + 5*r**4)/r**13)
        Z60 = (-Z/13)*(1/32)*np.sqrt(13/np.pi)*((231*z**6 - 315*z**4*r**2 + 105*z**2*r**4 - 5*r**6)/r**13)
        Zc61 = (-Z/13)*(-1/8)*np.sqrt(273/(4*np.pi))*(x*z*(33*z**4 - 30*z**2*r**2 + 5*r**4)/r**13)
        Zc62 = (-Z/13)*(1/64)*np.sqrt(2730/np.pi)*((x**2 - y**2)*(33*z**4 - 18*z**2*r**2 + r**4)/r**13)
        Zc63 = (-Z/13)*(-1/32)*np.sqrt(2730/np.pi)*((x**3 - 3*x*y**2)*(11*z**3 - 3*z*r**2)/r**13)
        Zc64 = (-Z/13)*(21/32)*np.sqrt(13/(7*np.pi))*((x**4 - 6*x**2*y**2 + y**4)*(11*z**2 - r**2)/r**13)
        Zc65 = (-Z/13)*(-np.sqrt(9009/(512*np.pi)))*((x**5 - 10*x**3*y**2 + 5*x*y**4)*z/r**13)
        Zc66 = (-Z/13)*(231/64)*np.sqrt(26/(231*np.pi))*((x**6 - 15*x**4*y**2 + 15*x**2*y**4 - y**6)/r**13)
        """   
        Bs22 = -(1/4)*np.sqrt(15/np.pi)*r_2*alpha*np.sum(Zs22)*e/e_0*1000
        Bs21 = -(1/4)*np.sqrt(15/np.pi)*r_2*alpha*np.sum(Zs21)*e/e_0*1000
        B20 = -(1/4)*np.sqrt(5/np.pi)*r_2*alpha*np.sum(Z20)*e/e_0*1000
        Bc21 = -(1/2)*np.sqrt(15/np.pi)*r_2*alpha*np.sum(Zc21)*e/e_0*1000
        Bc22 = -(1/4)*np.sqrt(15/np.pi)*r_2*alpha*np.sum(Zc22)*e/e_0*1000

        B22 = Bs22 + Bc22
        B21 = Bs21 + Bc21
        
        Bs44 = -(3/16)*np.sqrt(35/np.pi)*r_4*beta*np.sum(Zs44)*e/e_0*1000
        Bs43 = -(3/8)*np.sqrt(70/np.pi)*r_4*beta*np.sum(Zs43)*e/e_0*1000
        Bs42 = -(3/8)*np.sqrt(5/np.pi)*r_4*beta*np.sum(Zs42)*e/e_0*1000
        Bs41 = -(3/4)*np.sqrt(5/(2*np.pi))*r_4*beta*np.sum(Zs41)*e/e_0*1000
        B40 = -(3/16)*np.sqrt(1/np.pi)*r_4*beta*np.sum(Z40)*e/e_0*1000
        Bc41 = -(3/4)*np.sqrt(5/(2*np.pi))*r_4*beta*np.sum(Zc41)*e/e_0*1000
        Bc42 = -(3/8)*np.sqrt(5/np.pi)*r_4*beta*np.sum(Zc42)*e/e_0*1000
        Bc43 = -(3/8)*np.sqrt(70/np.pi)*r_4*beta*np.sum(Zc43)*e/e_0*1000
        Bc44 = -(3/16)*np.sqrt(35/np.pi)*r_4*beta*np.sum(Zc44)*e/e_0*1000
        ...
        B41 = Bs41 + Bc41
        """
        Bs66 = -(231/64)*np.sqrt(26/(231*np.pi))*r_6*gamma*np.sum(Zs66)*e/e_0*1000
        Bs65 = -np.sqrt(9009/(512*np.pi))*r_6*gamma*np.sum(Zs65)*e/e_0*1000
        Bs64 = -(21/32)*np.sqrt(13/(7*np.pi))*r_6*gamma*np.sum(Zs64)*e/e_0*1000
        Bs63 = -(1/32)*np.sqrt(2730/np.pi)*r_6*gamma*np.sum(Zs63)*e/e_0*1000
        Bs62 = -(1/64)*np.sqrt(2730/np.pi)*r_6*gamma*np.sum(Zs62)*e/e_0*1000
        Bs61 = -(1/8)*np.sqrt(273/(4*np.pi))*r_6*gamma*np.sum(Zs61)*e/e_0*1000
        B60 = -(1/32)*np.sqrt(13/np.pi)*r_6*gamma*np.sum(Z60)*e/e_0*1000
        Bc61 = -(-1/8)*np.sqrt(273/(4*np.pi))*r_6*gamma*np.sum(Zc61)*e/e_0*1000
        Bc62 = -(1/64)*np.sqrt(2730/np.pi)*r_6*gamma*np.sum(Zc62)*e/e_0*1000
        Bc63 = -(-1/32)*np.sqrt(2730/np.pi)*r_6*gamma*np.sum(Zc63)*e/e_0*1000
        Bc64 = -(21/32)*np.sqrt(13/(7*np.pi))*r_6*gamma*np.sum(Zc64)*e/e_0*1000
        Bc65 = -(-np.sqrt(9009/(512*np.pi)))*r_6*gamma*np.sum(Zc65)*e/e_0*1000
        Bc66 = -(231/64)*np.sqrt(26/(231*np.pi))*r_6*gamma*np.sum(Zc66)*e/e_0*1000
    ...
    def SOM(self,ion,L,S,d,Z,rho,Qeff):
        ...
        #rho should be in between 0.1 ~ 0.3 (Co in octahedral should be 0.13 to 0.16) and Qeff should be 0.6 ~ 1
...
class J():
    def __init__(self,Jvalue):
        self.J = Jvalue
        if Jvalue == 0 or 1 or 2 or 3 or 4 or 5 or 6 or 7 or 8 or 9:
            self.J_array = np.arange(-self.J,self.J+1)
